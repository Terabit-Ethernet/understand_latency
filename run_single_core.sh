...
for i in "${flows[@]}"
do
  ./cfs-both.sh $i temp/ 64
  mkdir temp/cfs_single_core_$i
  mv temp/*.log temp/cfs_single_core_$i
  ./linux-both.sh $i temp/ 64
  mkdir temp/linux_single_core_$i
  mv temp/*.log temp/linux_single_core_$i
#  ./ours-both.sh $i temp/ 64
#  mkdir temp/our_single_core_$i
#  mv temp/*.log temp/our_single_core_$i
done

for i in "${flows[@]}"
do
mkdir temp/$i
./parse-netperf.py temp/our_single_core_$i $i > temp/$i/nrfs_latency
./parse-netperf.py temp/linux_single_core_$i $i > temp/$i/arfs_latency
