...
for i in `seq 1 $N`; do
	...
	#sudo taskset -c 0 netperf -H 192.168.10.146 -t TCP_RR -B "-b $QD" -l 100 -j -p $DPORT -- -b $QD -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
