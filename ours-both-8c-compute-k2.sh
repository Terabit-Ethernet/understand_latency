...
for i in `seq 1 $N`; do
        ...
        ssh jaehyun\@128.84.155.146 -t "sudo taskset -c $TASKSET nice -n -20 netserver -p $((DPORT))"
done

sleep 3

for i in `seq 1 $N`; do
	sudo taskset -c $TASKSET netperf -H 192.168.10.125 -t TCP_RR -l 600 -j -p $DPORT -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
...
for i in `seq 1 8`; do
        ssh jaehyun\@128.84.155.146 -t "sudo taskset -c $TASKSET2 nice -n 19 /home/jaehyun/latency/compute" > $DIR/compute-$i-$N.log &
        PIDS2="$PIDS2 $!"
        echo "pid2 $PIDS2"
done
