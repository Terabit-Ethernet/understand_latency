...
for i in `seq 1 $N`; do
        ...
        ssh jaehyun\@128.84.155.146 -t "sudo /home/qizhe/caladan/apps/bench/netperf  /home/qizhe/caladan/server_ma_$i.config server $port" &
done
...
for i in `seq 1 $N`; do
	...
	addr=$((i + 127))
	port=$((8080 + i - 1))
	sudo /home/qizhe/caladan/apps/bench/netperf  /home/qizhe/caladan/client_ma_"$i".config tcprr 192.168.10."$addr" 1 180 $SIZE $port 1 &
	PIDS="$PIDS $!"
...
for i in `seq 1 8`; do
ssh jaehyun\@128.84.155.146 -t "sudo -s; cd /home/qizhe/latency;sudo /home/qizhe/caladan/apps/bench/compute_md /home/qizhe/caladan/compute_ma.config 1 1 $(($i-1))" &
PIDS2="$PIDS2 $!"
# echo "pid2 $PIDS2"
done
