...
ssh jaehyun\@128.84.155.146 -t "sudo taskset -c 0,28 nice -n -20 netserver -p $((DPORT))"

sleep 3

for i in `seq 1 $N`; do
	sudo taskset -c 0,28 netperf -H 192.168.10.125 -t TCP_RR -l 600 -f g -j -p $DPORT -- -r $SIZE,$SIZE -b $IODEPTH,$IODEPTH -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
