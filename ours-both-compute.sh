...
mkdir -p $DIR
for i in `seq 1 $N`; do
	sudo taskset -c 0 nice -n -20 netperf -H 192.168.10.146 -t TCP_RR -l 500 -f g -j -p $DPORT -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
