...
1. Download Linux 6.12.66 (EEVDF) kernel from [Linux](https://kernel.org)
    ```sh
    cd ~
...
1. Apply our ACCa and PCSched patch for Linux 6.12.66 kernel:
    ```sh
    git apply --check ../latency/linux-6.12-latency.patch
    git apply ../latency/linux-6.12-latency.patch
    ```

1. Compile the kernel following the above directions. Note you should disable IRQ time accounting for `linux-6.12.66` kernel, while enabling it for `linux-6.12.66-latency` kernel. Please see the kernel repo---the installation is similar.

1. Run the Figure 3-4 part experiment under defaut EEVDF kernel (`linux-6.12.66`) and our customized EEVDF kernel (`linux-6.12.66-latency`).

#### Evaluation and Data Parse

1. Please refer to Figure 3-4 part for latency-throughput curve.
