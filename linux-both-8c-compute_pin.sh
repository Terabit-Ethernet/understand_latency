...
for i in `seq 1 $N`; do
        ...
        ssh jaehyun\@128.84.155.146 -t "sudo taskset -c $(( (i * 4) % 32)) nice -n -20 netserver -p $((DPORT + i % N))"
done

sleep 3

for i in `seq 1 $N`; do
	sudo taskset -c $(( (i * 4) % 32)) netperf -H 192.168.10.125 -t TCP_RR -l 600 -j -p $((DPORT + (i) % N)) -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
