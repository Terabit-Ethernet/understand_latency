...
#mkdir -p $DIR
for i in `seq 1 4`; do
	sudo taskset -c 0 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5001 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
...
for i in `seq 5 8`; do
        sudo taskset -c 4 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5002 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done

for i in `seq 9 12`; do
        sudo taskset -c 8 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5003 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done

for i in `seq 13 16`; do
        sudo taskset -c 12 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5004 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done

for i in `seq 17 20`; do
        sudo taskset -c 16 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5005 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done

for i in `seq 21 24`; do
        sudo taskset -c 20 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5006 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done

for i in `seq 25 28`; do
        sudo taskset -c 24 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5007 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done

for i in `seq 29 32`; do
        sudo taskset -c 28 netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p 5008 -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
        PIDS="$PIDS $!"
        echo "pid $PIDS dport $DPORT"
done
