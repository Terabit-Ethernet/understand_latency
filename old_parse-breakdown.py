...
for port, ts in tails.items():
    tail[port] = {}
    for k, v in ts.items():
        tail[port][k] = sorted(v)[round(0.99 * len(v))]
...
print("\t{}".format("\t".join("avg\ttail" for _ in categories)))
for port in avg:
    print("{}\t{}".format(port, "\t".join("{}\t{}".format(avg[port][c], tail[port][c]) for c in categories)))
