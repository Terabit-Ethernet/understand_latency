...
sudo sysctl -w net.core.latency_breakdown_log=997
mkdir -p $DIR
for i in `seq 1 $N`; do
	...
	sudo taskset -c 0 netperf -H 192.168.10.146 -t TCP_RR -l 10 -j -p $DPORT -- -r $SIZE,$SIZE -o throughput,mean_latency,p50_latency,p99_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
