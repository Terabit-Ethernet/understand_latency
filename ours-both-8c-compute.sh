...
ssh jaehyun\@128.84.155.146 -t 'sudo sysctl -w net.core.latency_breakdown_nrfs=3'
...
mkdir -p $DIR
for i in `seq 1 $N`; do
	sudo taskset -c $TASKSET netperf -H 192.168.10.146 -t TCP_RR -l 100 -j -p $DPORT -- -r $SIZE,$SIZE -o throughput,mean_latency,p99_latency,p999_latency &> $DIR/netperf-$i-$N.log&
	PIDS="$PIDS $!"
